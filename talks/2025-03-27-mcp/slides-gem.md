...
本日は「MCP世界への招待」というテーマでお話させていただきます。

AIエージェントの未来を大きく変える可能性を秘めた技術について、
皆さんと一緒に見ていきたいと思います。
...
グンタ・ブルンナーと申します。
サイバーエージェントに2012年から所属していまして、
現在はCTO統括室のDeveloper Productivity室で働いています。

AIやUX、デザイン、パフォーマンスなどに興味があって、
趣味は音楽、映画、お寿司、旅行など色々あります。

最近は個人的にAIアートも始めました。
興味のある方は、ぜひTwitterやGitHub、Zennなどでフォローしてください！
-->
...
<!-- 
スピーカーノート：
サイバーエージェントでの12年間、色々な部署を経験してきました。

最初はメディア部門で、
ユーザー体験やサービスの作り方について学びました。

次にゲーム部門では、
フロントエンドの技術やパフォーマンスの最適化に取り組みました。

その後、横軸部門で開発生産性の向上に携わり、
今はここが私のメインフィールドになっています。

そして最近はAI部門でも活動して、
生成AIの実践的な活用方法を探求しています。

こうした様々な経験を経て、
今は「フルスタックAIエンジニア・デザイナー」として
幅広い視点からプロダクト開発に取り組んでいます。
-->
...
今日お話しするのは「Model Context Protocol」、略して「MCP」です。

これは、AIエージェントが外部のツールやデータと
連携するための新しい標準プロトコルなんです。

このMCPが、私たちAIエンジニアの働き方や、
AIを活用したプロダクト開発に、
どのような可能性をもたらすのか。

その世界に皆さんをご招待したいと思います。
-->
...
<!--
スピーカーノート：
皆さん、最近のAI技術の進化は目覚ましいですよね。

GitHub Copilotや、Cursor、Claude Codeなど、
AIを活用した開発支援ツールが急速に普及しています。

こういったツールを使ってコードを書くことが、
もはや当たり前になってきました。

でも考えてみてください。
これらのAIが本当に便利になるためには、
コードを生成するだけじゃなくて、
プロジェクトの背景やコンテキストを理解して、
様々なツールと連携できることが大切なんです。

例えば「GitHubのIssueをもとにコードを書いて、
それをPRで提出して、Slackで通知する」といった
一連の流れをサポートできれば最高ですよね。

ここで重要になるのが、今日お話しするMCPなんです。
-->
...
<!--
スピーカーノート：
この発表は、主に以下のような方々に聞いていただきたいと思っています。

まず、LLMやAIエージェントを活用した開発に
興味を持っているエンジニアの方々。

次に、AIツールをもっと連携させたい、
拡張性を高めたいと考えている方々。

そして、開発の効率化や次世代のワークフローを
模索している皆さんにも参考になると思います。

ただ、AIやプログラミングに興味があれば、
技術的なバックグラウンドがなくても
十分に理解できる内容にしていますので、
どうぞリラックスして聞いてください。
-->
...
<!--
スピーカーノート：
現状、AIエージェントに外部ツールを連携させるのは
かなり大変なんですよね。

例えば、高品質なツールを見つけるだけでも一苦労。
色々検索してみても、本当に使えるものと
そうでないものが混在していて、判断が難しい。

それに、各ツールの品質にもばらつきがあって、
標準的な評価基準もない状態です。

さらに、ツールごとに設定方法が違うので、
連携させるたびに新しい設定方法を学ばないといけません。

そして一番の問題は、各社が独自の仕様で
ツールを提供していることで、
あるAIツールで使えた連携が、別のAIツールでは
使えないということがよくあります。
...
<!-- 
スピーカーノート：
では、MCPとは具体的に何なのでしょうか？

まず、MCPはAnthropic社が提唱して、
オープンソースコミュニティと一緒に開発している
オープンスタンダードです。

簡単に言うと、AIアプリケーション（例えばCursorや
Claude Desktopなどのクライアント）と、
外部のツールやデータ（例えばGitHubやファイルシステムなどの
サーバー）が対話するための標準的な方法を定めたものです。

これをわかりやすく例えると、
「AIのためのUSB-C」とも言えます。

USB-Cがどんなデバイスでも同じコネクタで
接続できるように、MCPはどんなAIツールでも
同じインターフェースで外部サービスと連携できる
ことを目指しています。
...
まず「クライアント」。これはAIアプリケーションのことで、
CursorやClaude Desktopなどの、ユーザーが直接使うアプリです。

次に「サーバー」。こちらはツールやデータを提供する側で、
例えばGitHub操作やファイルシステム操作などの機能を
提供します。

「トランスポート」は、クライアントとサーバーが
どうやって通信するかという方法です。
標準入出力（stdio）や、HTTP通信などが使われます。

そして「メッセージ」は、両者がやり取りする情報のフォーマットで、
JSON-RPCという形式が採用されています。

この4つの要素が組み合わさることで、
AIアプリケーションは様々なツールを
統一的なインターフェースで利用できるようになるんです。

実際のところ、「JSON-RPC？トランスポート？」と
難しく感じるかもしれませんが、
エンドユーザーやAIエンジニアとしては、
この仕組みを詳細に理解する必要はありません。

重要なのは、これによって様々なツールが
簡単に連携できるようになるということです。
-->
...
<!-- 
スピーカーノート：
MCPの重要な概念として、「リソース」と「ツール」の
区別があります。

「リソース」は、アプリケーションが管理していて、
AIモデルに提供されるコンテキスト情報です。
例えば、IDEが開いているファイルの内容や、
ユーザーが選択しているコードの範囲などが該当します。

一方「ツール」は、AIモデルが自分から
呼び出して利用できる機能です。
例えば「ファイルに書き込む」「APIを叩く」
といった操作が該当します。

そして「プロンプト」は、ユーザーが入力した指示です。

この図を見ていただくと分かりやすいんですが、
リソースはAIに「見せる情報」、
ツールはAIに「できること」と捉えると良いでしょう。

この区別によって、AIモデルは「今どんな情報が
見られるのか」を理解した上で、
「どんな操作ができるのか」を認識し、
適切な行動を取れるようになります。

MCPの大きな特徴は、この動的なコンテキストアクセスを
中心に設計されているところです。
-->
...
MCPは既存の技術から着想を得ています。
特に、開発者の皆さんにはおなじみの
「Language Server Protocol（LSP）」との類似点が多いです。

LSPというのは、VSCodeなどのIDEが、
言語固有の機能（コード補完や定義ジャンプなど）を
提供するサーバーと通信するための規約です。

MCPはこのLSPの考え方をAIツール連携に応用しています。

LSPがIDEと言語サーバーを分離したように、
MCPはAIクライアントとツールサーバーを分離します。

また、LSPと同様にJSON-RPCという形式で通信したり、
「このサーバーにはどんな機能があるか」を
動的に発見する仕組みも取り入れています。

この図はLSPの仕組みを表したものですが、
MCPも基本的に同じような構造になっています。

これにより、クライアントは特定のツール実装に依存せず、
サーバー側も様々なクライアントから
利用される可能性が生まれます。

LSPが多くのIDE開発を変革したように、
MCPもAIツール連携の世界を変える可能性を秘めています。
-->
...
まず、IDEやエディタの分野では、
Cursorを筆頭に、Windsurfや
Zed、Codyなどが対応しています。

デスクトップアプリでは、Claude Desktopが
対応していますが、最近の大きなニュースとして、
OpenAIがChatGPTデスクトップアプリでの
MCP対応を発表しました！

これはすごいことで、MCPがもはや開発者だけの
技術ではなく、一般ユーザーにも恩恵をもたらす
可能性が高まったことを意味します。

CLIでは、Claude CodeやClineなどが利用可能です。

そして、SDK方面では、VercelのAI SDKや
Mastraなどのフレームワークを使えば、
自分のアプリにMCPクライアント機能を
簡単に組み込むことができます。

このように、様々な形でMCPクライアントが
提供されているので、自分の好みや用途に合わせて
選ぶことができます。
-->
...
<!-- 
スピーカーノート：
次にMCPサーバーの例を見ていきましょう。

まず、Anthropic自身が提供している
公式リファレンス実装があります。
これには、ファイルシステム操作、GitHub連携、
会話履歴の管理など、基本的ながら強力な
ツールが19種類も含まれています。

例えば、Sequential Thinkingというサーバーは、
AIの思考プロセスを段階的に進めることで、
より複雑な問題解決を可能にします。

一方、サードパーティからも様々なサーバーが
登場しています。
例えば、Mastraのドキュメント検索サーバーや、
Microsoftが開発したPlaywrightを使った
ブラウザ操作サーバーなどがあります。

さらに後ほど紹介しますが、レジストリを通じて、
SlackやNotion、Linearなど、何千もの
既存サービスに接続できるサーバーも
利用可能になっています。

このように、MCPエコシステムは
サーバー側も非常に豊かになりつつあります。
-->
...
<!-- 
スピーカーノート：
MCPサーバーが増えてくると、「どうやって目的のサーバーを
見つければいいの？」という問題が出てきます。

また、サーバーの品質はどうやって判断すればいいのか、
設定方法はどう標準化すればいいのか、といった
課題も生じます。

こういった課題に対応するのが「レジストリ」です。

レジストリは、いわばMCPサーバーのカタログや
マーケットプレイスのようなもので、
サーバーを見つけやすくしたり、設定を
簡単にしたりする役割を担っています。

現在、主要なレジストリとして、Composioや
Smithery、Zapier MCPなどがあります。

特にZapier MCPは7,000以上のアプリと
30,000以上のアクションに接続できるので、
普段使っているほとんどのSaaSツールと
連携できると言っても過言ではありません。

これらのレジストリを利用することで、
「このAIアプリからSlackに通知を送りたい」といった
ニーズに簡単に応えられるようになります。

なお、現在は公式なレジストリ仕様の策定も
進められており、将来的にはさらに統一された
エコシステムになることが期待されています。
-->
...
スピーカーノート：
MCPは活発に進化しています。
2025年3月26日、つまりほんの先日の改訂で、
いくつかの重要な変更がありました。

まず大きな変更点として、OAuthベースの
認証フレームワークが追加されました。
これは「Remote MCP」、つまりインターネット経由で
安全にMCPサーバーに接続するための基盤となります。

次に、新しい「Streamable HTTP」というトランスポートが
導入されました。これは従来のHTTP+SSEに代わるもので、
より柔軟な通信が可能になります。

特に重要なのは、このトランスポートにより
サーバーを完全にステートレスに実装できるようになったことです。
これはサーバーレス環境での運用を容易にし、
実装・運用コストを大幅に下げる可能性があります。

その他、複数のリクエストをまとめて送信する
「JSON-RPC Batching」や、
ツールの挙動をより詳細に記述できる
「Tool Annotations」なども追加されました。

こうした機能強化により、MCPはよりセキュアで
柔軟性の高いプロトコルへと進化しています。
-->
...
<!--
スピーカーノート：
MCPの公式ロードマップを見てみましょう。
2025年前半に予定されている開発内容です。

最優先事項として引き続き「Remote MCP」の
実現が挙げられています。
先ほどお話しした通り、認証とステートレス運用は
最近の仕様改訂で大きく前進しましたね。

今後は「サービスディスカバリ」、つまり
どうやってMCPサーバーを見つけるかという
標準的な方法の策定が焦点となります。

また、よりわかりやすいリファレンス実装の提供や、
サーバーの配布・発見を容易にする仕組みづくりも
計画されています。

エージェントサポートの面では、複数のAIが階層的に
連携するシステムや、よりインタラクティブな
ワークフローの実現を目指しています。

その他にも、テキスト以外の音声や動画などにも
対応する拡張や、標準化団体での検討も
進められています。

もちろん、これはあくまで公式のロードマップで、
コミュニティによる独自の拡張も
活発に行われているところです。
-->
...
<!-- 
スピーカーノート：
それでは、MCPの実用例を見ていきましょう。

まず最初に、Vercel社が提供するAI SDKでの
利用例です。

AI SDKは「experimental_createMCPClient」という
関数を通じてMCPクライアント機能を提供しています。

このコードを見ていただくと、最初にMCPクライアントを
作成しています。ここでは、SSE経由でリモートの
MCPサーバーに接続しています。

次に「mcpClient.tools()」でそのサーバーが提供している
ツール定義を取得しています。

そして、普段AI SDKで使っている「generateText」関数に、
そのMCPツールを渡しています。これだけで、AIモデルは
そのMCPツールを利用できるようになります。

右側のメリットを見ると、これによって数百の既存MCPツールに
アクセスできるようになりますし、ユーザーが独自の
MCPサーバーを接続してアプリを拡張することも可能になります。

また、ローカルのMCPサーバーにも、リモートのサーバーにも
両方接続できるので、用途に応じて使い分けられます。

たった数行のコードで、アプリケーションの機能を
大幅に拡張できるのが、MCPの大きなメリットですね。
-->
...
<!-- 
スピーカーノート：
次に、エージェント開発フレームワークである
Mastraでの利用例を見てみましょう。

Mastraは「MastraMCPClient」というクラスを提供していて、
AI SDKと同様に様々なMCPサーバーに接続できます。

このコード例では、ローカルでnpxコマンドを使って
「Sequential Thinking」というサーバーを起動し、
そこに接続しています。

接続後、サーバーからツール定義を取得して、
「agent.stream」の「toolsets」オプションで
エージェントに渡しています。

これによって、エージェントはSequential Thinking
サーバーの機能を使って、より複雑な思考プロセスを
実行できるようになります。

右側に書かれているように、Mastraはさらに進んで、
将来的にMCPレジストリと連携し、サーバーの検索から
設定、接続までをよりシームレスに行えるようにする
構想を持っています。

また、面白い試みとして、Mastra自身のドキュメントや
ブログ記事などを検索できるMCPサーバーも提供しています。

これをCursorなどの対応IDEに設定すると、
IDE内で「Mastraで〇〇するにはどうすればいい？」
といった質問をAIエージェントに投げかけるだけで、
関連ドキュメントを基に回答を得られるようになります。

このように、MCPはフレームワークの開発体験も
大きく向上させる可能性を秘めています。
-->
...
<!-- 
スピーカーノート：
MCPの真価は、複数のツールを組み合わせた
ワークフローの自動化にあります。

例えば、新機能開発のシナリオを考えてみましょう。

まず要件把握の段階では、AIエージェント（Cursorなど）が、
MCPサーバーを通じてJiraやGitHubからチケット情報を取得し、
Figmaから関連デザインを取得して、開発要件を理解します。

次に実装段階では、これらの情報をコンテキストとして
コードを生成します。もし実装中に仕様の疑問点があれば、
MCP経由でJiraチケットを更新したり、
Slackで担当者に質問をしたりできます。

実装が完了したら、Gitサーバーに接続して変更内容に
基づいたコミットメッセージを生成し、GitHubサーバーに
接続してPRを作成、関連チケットへのリンク付けや
ラベル設定まで自動で行えます。

さらに、デバッグ中にブラウザのコンソールログを取得したり、
ローカルのタスク管理ツールと連携したり、開発完了を
チームに通知したりと、開発者の日常的な雑務の多くを
MCP連携によって自動化できる可能性があります。

これは、エンジニアが本質的なコーディング作業に
より集中できる環境を実現する、
パラダイムシフトと言えるでしょう。

皆さんの日々の開発作業で、「これ、自動化できたら
どれだけ楽だろう」と思うことはありませんか？
そういった悩みの多くは、
MCPで解決できるかもしれません。
-->
...
<!-- 
スピーカーノート：
AI駆動開発において、生成されるコードの品質を
左右する最も重要な要素は何でしょうか？

それは、AIに与える「コンテキスト」です。

ここで重要になるのが「Project as Code（PaC）」
という考え方です。

これは、ソースコードだけでなく、要件定義、設計書、
開発ルール、インフラ構成など、プロジェクトに関する
あらゆる情報をコード（テキストファイル）として
バージョン管理するアプローチです。

なぜこれが重要かというと、AIとのコミュニケーションは
主にテキストベースであり、AIが理解しやすい形で
プロジェクトの全体像や制約条件を与える必要があるからです。

例えば、プロジェクト固有のコーディング規約を
Markdownで記述しておけば、AIはそれを参照して
規約に沿ったコードを生成できます。

PaCによって管理されたプロジェクト情報は、
MCPを通じてAIエージェントに最適なコンテキストとして
提供できます。

具体的には、Cursorの @workspace 機能で
プロジェクト内のドキュメントを参照させたり、
専用のMCPサーバーを用意して特定のドキュメント群を
提供することで、AIはプロジェクト固有のルールや
仕様を正確に踏まえたコード生成を行えるようになります。

PaCとMCPは、AI駆動開発の精度を高める上で
非常に相性の良い組み合わせと言えるでしょう。
-->
...
何かご質問がありましたら、ぜひお聞かせください。

また、今日の話に関連する内容や
AIエンジニアリングに関する情報は、
TwitterやGitHub、Zennなどで発信していますので、
ご興味があればフォローしていただければ嬉しいです。

今後もMCPやAIエージェント開発に関する
新しい発見や知見を共有していきたいと思います。
...
スピーカーノート：
MCPは、登場からまだ日が浅いものの、
急速に支持を広げ、「デファクトスタンダード」の
地位を確立しつつあります。

なぜこれほど急速に広まったのか、
その理由をいくつか考えてみましょう。

まず、既存のAPI仕様とは異なり、AIエージェントが
動的なコンテキストを扱ったり、ツールとリソースを
区別したりといった、AIネイティブな要求に
応える形で設計されています。

次に、AnthropicというAI業界の主要プレイヤーが
バックについていながらも、オープンスタンダードとして
公開されていることです。これにより、特定のベンダーに
ロックインされる懸念が少なく、多くの企業が
安心して採用できます。

最近のOpenAIのMCP採用表明は、この流れを
決定づけたと言えるでしょう。

また、開発者からの評価が高いClaudeを提供している
Anthropicのブランド力や、実績のあるLSPを
ベースにした堅実な設計も成功要因です。

さらに、Anthropic自身がクライアント、サーバー、
ツールを開発・利用する「ドッグフーディング」を
行っていることで、現実的なユースケースに
基づいた設計になっています。

最小限の仕様からスタートし、明確なロードマップを
示しながら段階的に進化している点も、
エコシステムの拡大に貢献しています。

こうした要因が重なり、MCPは今、最も有望な
AIツール連携の標準として注目されているのです。
-->
...
テキスト以外の音声や画像、動画などを扱うMCPサーバーも増えていくでしょう。
...
<!--
スピーカーノート：
本日は、Model Context Protocol (MCP) について、その概要からエコシステム、最新動向、そして未来の
可能性まで幅広くご紹介しました。
MCPは、AIエージェント開発におけるツール連携の課題を解決し、開発ワークフローを大きく変革する
可能性を秘めた、今最も注目すべき技術の一つです。
